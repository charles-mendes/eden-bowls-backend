...
function registerAdminPrivacyRoutes(app, dependencies, { requirePermission, handle }) {
  app.get('/api/v1/admin/privacy/requests', requirePermission('privacy.requests.read'), async (request, response, next) => {
    await handle(response, next, async () => {
      ...
      const result = await privacyService.listAdminRequests(query, pagination);
  ...
  app.post('/api/v1/admin/privacy/requests', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
  ...
  app.get('/api/v1/admin/privacy/requests/:id', requirePermission('privacy.requests.read'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      return privacyService.getAdminRequest(parseRequestId(request.params.id));
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/in-progress', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      const actorId = request.adminIdentity && Number(request.adminIdentity.userId);
      return privacyService.setInProgress(parseRequestId(request.params.id), actorId);
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/extend', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      const { parseExtendInput } = require('../validators/privacy.validator');
      const payload = parseExtendInput(request.body || {});
      const actorId = request.adminIdentity && Number(request.adminIdentity.userId);
      return privacyService.extendOnce(parseRequestId(request.params.id), payload.reason, actorId);
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/complete', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      const { parseCompleteInput } = require('../validators/privacy.validator');
      const payload = parseCompleteInput(request.body || {});
      const actorId = request.adminIdentity && Number(request.adminIdentity.userId);
      return privacyService.completeRequest(parseRequestId(request.params.id), payload.note, actorId);
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/reject', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      const { parseCompleteInput } = require('../validators/privacy.validator');
      const payload = parseCompleteInput(request.body || {});
      const actorId = request.adminIdentity && Number(request.adminIdentity.userId);
      return privacyService.rejectRequest(parseRequestId(request.params.id), payload.note, actorId);
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/send-verification', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      return privacyService.sendIdentityVerification(parseRequestId(request.params.id));
    });
  });

  app.post('/api/v1/admin/privacy/requests/:id/verify-identity', requirePermission('privacy.requests.write'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      const actorId = request.adminIdentity && Number(request.adminIdentity.userId);
      return privacyService.markIdentityVerified(parseRequestId(request.params.id), actorId);
    });
  });

  app.get('/api/v1/admin/privacy/requests/:id/export', requirePermission('privacy.requests.read'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      return privacyService.downloadAdminPackage(parseRequestId(request.params.id));
    });
  });

  app.get('/api/v1/admin/users/:userId/privacy', requirePermission('users.read'), async (request, response, next) => {
    await handle(response, next, async () => {
      const privacyService = requirePrivacyService(dependencies);
      ...
      return privacyService.getUserPrivacySnapshot(userId);
    });
  });
}
